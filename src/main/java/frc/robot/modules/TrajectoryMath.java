...
public class TrajectoryMath {
   ...
   * @return
   */
  public static double getVelocityFromDistance(double distance, double goalHeight) {
    double a = GRAVITY / 2 * Math.pow(distance, 2);
    double b = (Math.sin(ANGLE_RADS) * Math.cos(ANGLE_RADS) * distance) 
        - (Math.pow(Math.cos(ANGLE_RADS), 2) * (goalHeight - SHOOTER_HEIGHT));
    double c = Math.sqrt(a / b);
    return c * 60 / (.67    * (2.0 / 12.0) * 2 * Math.PI);
  }

  public static double getDistanceFromPitch(double pitch, double goalHeight) {
    return Math.abs((goalHeight - SHOOTER_HEIGHT) / Math.tan(Math.toRadians(pitch) + ANGLE_RADS));
  }
}
